...
namespace SharpDXExamples.Examples.FpsCpuUsageTimes {
    public class Text {
        ...
        public bool Initialize(Device device, DeviceContext deviceContext, int screenWidth, int screenHeight, Matrix baseViewMatrix) {
            ...
            font = new Font();
            if(!font.Initialize(device, @"Examples\FpsCpuUsageTimes\Data\Font.data", @"Examples\FpsCpuUsageTimes\Data\Font.png"))
                return false;
        ...
        public bool SetFps(int fps, DeviceContext deviceContext) {
            ...
            if(!UpdateSentence(ref sentence1, $"FPS: {fps}", 20, 20, red, green, blue, deviceContext))
                return false;

            return true;
        }

        public bool SetCpu(float cpu, DeviceContext deviceContext) {
            if(!UpdateSentence(ref sentence2, $"CPU: {cpu.ToString("0.00")}%", 20, 40, 0.0f, 1.0f, 0.0f, deviceContext))
                return false;
