...
namespace SharpDXExamples.Examples.AmbientLighting {
    public class Graphics {
        ...
        public bool Initialize(int screenWidth, int screenHeight, IntPtr hwnd) {
            ...
            model = new Model();
            if(!model.Initialize(direct3D.Device, @"Examples\AmbientLighting\Data\Cube.data", @"Examples\AmbientLighting\Data\Seafloor.png"))
                return false;
        ...
        bool Render(float rotation) {
            ...
            if(!lightShader.Render(direct3D.DeviceContext, model.IndexCount, world, view, projection, model.ObjectTexture, light.Direction, light.AmbientColor, light.DiffuseColor))
                return false;
            direct3D.EndScene();
