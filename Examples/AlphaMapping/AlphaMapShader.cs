...
namespace SharpDXExamples.Examples.AlphaMapping {
    public class AlphaMapShader {
        ...
        public bool Initialize(Device device) {
            if(!InitializeShader(device, @"Examples\AlphaMapping\Shaders\AlphaMap.vs", @"Examples\AlphaMapping\Shaders\AlphaMap.ps"))
                return false;
        ...
        bool InitializeShader(Device device, string vsFileName, string psFileName) {
            try {
                var compileVertexShaderResult = ShaderBytecode.CompileFromFile(vsFileName, "AlphaMapVertexShader", "vs_5_0", ShaderFlags.EnableStrictness);
                vertexShader = new VertexShader(device, compileVertexShaderResult.Bytecode);

                var compilePixelShaderResult = ShaderBytecode.CompileFromFile(psFileName, "AlphaMapPixelShader", "ps_5_0", ShaderFlags.EnableStrictness);
