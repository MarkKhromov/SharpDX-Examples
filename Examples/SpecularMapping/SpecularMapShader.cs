...
namespace SharpDXExamples.Examples.SpecularMapping {
    public class SpecularMapShader {
        ...
        public bool Initialize(Device device) {
            if(!InitializeShader(device, @"Examples\SpecularMapping\Shaders\SpecularMap.vs", @"Examples\SpecularMapping\Shaders\SpecularMap.ps"))
                return false;
        ...
        public bool Render(DeviceContext deviceContext, int indexCount, Matrix world, Matrix view, Matrix projection, ShaderResourceView[] textureArray, Vector3 lightDirection, Color4 diffuseColor, Vector3 cameraPosition, Color4 specularColor, float specularPower) {
            if(!SetShaderParameters(deviceContext, world, view, projection, textureArray, lightDirection, diffuseColor, cameraPosition, specularColor, specularPower))
                return false;
        ...
        bool InitializeShader(Device device, string vsFileName, string psFileName) {
            try {
                var compileVertexShaderResult = ShaderBytecode.CompileFromFile(vsFileName, "SpecularMapVertexShader", "vs_5_0", ShaderFlags.EnableStrictness);
                vertexShader = new VertexShader(device, compileVertexShaderResult.Bytecode);

                var compilePixelShaderResult = ShaderBytecode.CompileFromFile(psFileName, "SpecularMapPixelShader", "ps_5_0", ShaderFlags.EnableStrictness);
                ...
                var cameraBufferDescription = new BufferDescription {
                    Usage = ResourceUsage.Default,
                    SizeInBytes = Utilities.SizeOf<CameraBufferType>(),
                    BindFlags = BindFlags.ConstantBuffer,
                    CpuAccessFlags = CpuAccessFlags.None,
                    OptionFlags = ResourceOptionFlags.None,
                    StructureByteStride = 0
                ...
                cameraBuffer = Buffer.Create(device, new[] { new CameraBufferType() }, cameraBufferDescription);
            } catch { return false; }
            return true;
        }
