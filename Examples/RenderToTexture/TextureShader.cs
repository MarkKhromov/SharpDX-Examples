...
namespace SharpDXExamples.Examples.RenderToTexture {
    public class TextureShader {
        ...
        public bool Initialize(Device device) {
            if(!InitializeShader(device, @"Examples\RenderToTexture\Shaders\Texture.vs", @"Examples\RenderToTexture\Shaders\Texture.ps"))
                return false;
        ...
        bool InitializeShader(Device device, string vsFileName, string psFileName) {
            try {
                var compileVertexShaderResult = ShaderBytecode.CompileFromFile(vsFileName, "TextureVertexShader", "vs_5_0", ShaderFlags.EnableStrictness);
                vertexShader = new VertexShader(device, compileVertexShaderResult.Bytecode);

                var compilePixelShaderResult = ShaderBytecode.CompileFromFile(psFileName, "TexturePixelShader", "ps_5_0", ShaderFlags.EnableStrictness);
