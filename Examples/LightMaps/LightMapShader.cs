...
namespace SharpDXExamples.Examples.LightMaps {
    public class LightMapShader {
        ...
        public bool Initialize(Device device) {
            if(!InitializeShader(device, @"Examples\LightMaps\Shaders\LightMap.vs", @"Examples\LightMaps\Shaders\LightMap.ps"))
                return false;
        ...
        bool InitializeShader(Device device, string vsFileName, string psFileName) {
            try {
                var compileVertexShaderResult = ShaderBytecode.CompileFromFile(vsFileName, "LightMapVertexShader", "vs_5_0", ShaderFlags.EnableStrictness);
                vertexShader = new VertexShader(device, compileVertexShaderResult.Bytecode);

                var compilePixelShaderResult = ShaderBytecode.CompileFromFile(psFileName, "LightMapPixelShader", "ps_5_0", ShaderFlags.EnableStrictness);
